  ...
  private init(): void {
    ...
    if (typeof chrome !== 'undefined' && chrome.runtime && chrome.runtime.onMessage) {
      chrome.runtime.onMessage.addListener((message: Message, sender: any, sendResponse: (response: any) => void) => {
        if (message.action === 'toggleEnhancement') {
          ...
          sendResponse({ status: 'toggled', enabled: this.isEnabled });
        }
      });
    }
  }

  private shouldEnhanceThisSite(): boolean {
